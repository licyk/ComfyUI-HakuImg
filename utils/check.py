...
def compatible_version_matcher(spec_version: str):
    ...
    if len(clean_release) == 0:
        logger.error('Invalid version for compatible release clause')
...
def parse_requirement_list(requirements: list) -> list:
    ...
    for requirement in requirements:
        ...
        logger.debug('Origin Python package name: %s', requirement)
    ...
    for p in package_list:
        p: str = p.lower().strip()
        logger.debug('Python package name after preprocess: %s', p)
        if not is_package_has_version(p):
            logger.debug('%s no version statement', p)
        ...
        if version_string_is_canonical(get_package_version(p)):
            ...
            logger.debug('%s version does not meet the standard', p)
...
def is_package_installed(package: str) -> bool:
    ...
    logger.debug(
        'Installed Python package detection: pkg_name: %s, env_pkg_version: %s, pkg_version: %s',
    ...
    if pkg_version is not None:
        ...
        if '===' in package or '==' in package:
            logger.debug('Include conditions: === / ==')
        ...
        logger.debug('%s need install', package)
        return False
