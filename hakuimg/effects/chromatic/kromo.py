...
def vertical_gaussian(data: np.ndarray, n: int) -> np.ndarray:
    ...
    width = data.shape[1]
    height = data.shape[0]
    padded_data = np.zeros((height + padding * 2, width), dtype=np.float64)
    padded_data[padding:-padding, :] = data
    ret = np.zeros((height, width), dtype=np.float64)
    kernel: np.ndarray = np.empty((0, 0), dtype=np.float64)
    old_radius = -1
    for i in range(height):
        ...
        ret[i, :] = np.sum(
            np.multiply(
                padded_data[padding + i - radius + 1 : padding + i + radius, :],
                kernel,
            ),
            axis=0,
        )
...
def add_chromatic(im: Image.Image, strength: float = 1, no_blur: bool = False) -> Image.Image:
    ...
    if no_blur:
        ...
        rfinal = Image.fromarray(np.asarray(rcartes, dtype=np.uint8), "L")
        gfinal = Image.fromarray(np.asarray(gcartes, dtype=np.uint8), "L")
        bfinal = Image.fromarray(np.asarray(bcartes, dtype=np.uint8), "L")
...
def add_jitter(im: Image.Image, pixels: int = 1) -> Image.Image:
...
def blend_images(im: Image.Image, og_im: Image.Image, alpha: float = 1, strength: float = 1) -> Image.Image:
